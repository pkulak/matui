...
impl Message {
    ...
    pub fn simple_flatten(&self) -> Vec<&Message> {
        let mut messages = vec![self];

        for r in &self.replies {
            messages.append(&mut r.simple_flatten());
        }

        messages
    }

    pub fn flatten(&self, width: usize) -> Vec<(usize, &Message)> {
        let width = 10.max(width);
        let mut messages = vec![(width, self)];

        for r in &self.replies {
            messages.append(&mut r.flatten(width - 2));
        }
